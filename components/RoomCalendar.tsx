...
import { 
  ...
  X, Save, Trash2, Edit, ExternalLink, Type, AlignLeft, LogIn, LogOut, 
  CheckCircle, Loader2, RefreshCw, MapPin
} from 'lucide-react';
      ...
      <div className="grid grid-cols-7 auto-rows-fr bg-white dark:bg-gray-800">
        {calendarGrid.map((day, idx) => (
          ...
          >
            {day.isCurrentMonth && (
              <>
                <div className={`text-sm font-medium mb-2 ${day.fullDate === new Date().toISOString().split('T')[0] ? 'bg-blue-600 text-white w-7 h-7 flex items-center justify-center rounded-full' : 'text-gray-700 dark:text-gray-300'}`}>
                  {day.date}
                </div>
              ...
              <div className="px-6 pt-4">
                <button 
                  onClick={() => {
                    ...
                    setEventForm({ ...eventForm, startDate: selectedDayDetail.fullDate });
